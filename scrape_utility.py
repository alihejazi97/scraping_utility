...
def download_video_file(download_bar, download_bar_index, movie, sampling_rate : float, audio_path):
    ...
    if not os.path.isfile(audio_path + 'all.wav'):
        ...
        urllib.request.urlretrieve(_download_link, _video_file_path, ShowProgressUrllib(f'downloading video {movie["title"]}'))
        ffmpeg.input(_video_file_path).output(audio_path + 'all.wav', ac=1, ar=sampling_rate).run()
